...
import com.feedify.database.repository.ReadLaterFeedItemUserRepository;
import com.feedify.rest.service.sortingandfiltering.CustomPagination;

@Service
public class ReadLaterFeedsSearchingService {
	@Autowired
	...
	private List<FeedItemUser> findFilteredAndOrderedFeedItemsUser(String q, User user, LocalDate startDate,
		...
		case ORDER_BY_ALL_CATEGORIES: {
			feedItemsUser = findFeedItemUsersByReadLaterOrderedByCategoryName(user, true, pageNumber, feedItemsUser);
			break;
		} // end case
	...
	private List<FeedItemUser> findFeedItemUsersByReadLaterOrderedByCategoryName(User user, boolean readLater,
	...
	private List<FeedItemUser> search(Long userId, String query, Integer pageNumber) {
		...
		List<FeedItemUserId> feedItemUsersIds = feedItemsIdsPage.stream().filter(feedItemId -> {
			Optional<FeedItemUser> opitonalFeedItemUser = readLaterFeedItemUserRepository
					.findByIdAndReadLater(new FeedItemUserId(feedItemId, userId), true);
		...
		List<FeedItemUser> feedItemUsers = feedItemUsersIds.stream().map(feedItemUsersId -> {
			Optional<FeedItemUser> opitonalFeedItemUser = readLaterFeedItemUserRepository
					.findByIdAndReadLater(feedItemUsersId, true);
