...
function loadFeeds() {
  ...
  if (searchTerm.trim().length !== 0) search();
...
window.addEventListener("scroll", () => {
  if (
    ...
    document.documentElement.scrollHeight - 2.5
  ) {
    loadFeeds();
  }
});
...
searchTerm.addEventListener("input", (e) => {
  let searchTermValue = document.getElementById("q").value;
  if (searchTermValue.trim().length !== 0) {
